/*
	...
	CS60038 : Assignment 1 (part b) 
	LKM For Deque
*/
#include "part1b_18CS10069.h"
...
static int32_t insert(Deque *deque, int32_t key)
{
	if (deque->count < deque->capacity){
		/* if number is even; insert from the end */
		if (key % 2 == 0){
			deque->arr[deque->count] = key;
		}
		else{
			/* if number is odd; insert from the front */
			int32_t i;
...
static ssize_t dev_write(struct file *file, const char* buf, size_t count, loff_t* pos){
	...
	if (deque_initialized){
		...
		ret = insert(htable_elem->global_deque, num); /* insert into deque */
		if (ret < 0){ 
...
static ssize_t dev_read(struct file *file, char* buf, size_t count, loff_t* pos){
	...
	first_deque_elem = remove(htable_elem->global_deque);
	if(sizeof(first_deque_elem) != count){
		/* invalid read - add back the removed element in the front */
		printk(KERN_INFO DEVICE ": (dev_read) (PID %d) first_deque_elem is %d; failed to send to user process because of invalid read", current->pid, first_deque_elem);
		insert(htable_elem->global_deque, first_deque_elem);
		return -EACCES;
	}
